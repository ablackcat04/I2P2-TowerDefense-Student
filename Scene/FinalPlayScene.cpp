...
const int FinalPlayScene::MapWidth = 20, FinalPlayScene::MapHeight = 13;
const int FinalPlayScene::BlockSize = 64;
const float FinalPlayScene::DangerTime = 7.61;
const Engine::Point FinalPlayScene::SpawnGridPoint = Engine::Point(-1, 0);
const Engine::Point FinalPlayScene::EndGridPoint = Engine::Point(MapWidth, MapHeight - 1);
const std::vector<int> FinalPlayScene::code = { ALLEGRO_KEY_UP, ALLEGRO_KEY_UP, ALLEGRO_KEY_DOWN, ALLEGRO_KEY_DOWN,
                                           ALLEGRO_KEY_LEFT, ALLEGRO_KEY_LEFT, ALLEGRO_KEY_RIGHT, ALLEGRO_KEY_RIGHT,
                                           ALLEGRO_KEY_B, ALLEGRO_KEY_A, ALLEGRO_KEYMOD_SHIFT, ALLEGRO_KEY_ENTER };
Engine::Point FinalPlayScene::GetClientSize() {
...
void FinalPlayScene::Update(float deltaTime) {
...
void FinalPlayScene::Draw() const {
    IScene::Draw();
    if (DebugMode) {
...
void FinalPlayScene::OnMouseDown(int button, int mx, int my) {
...
void FinalPlayScene::OnMouseMove(int mx, int my) {
...
void FinalPlayScene::OnMouseUp(int button, int mx, int my) {
...
void FinalPlayScene::OnKeyDown(int keyCode) {
...
int FinalPlayScene::GetMoney() const {
    return money;
}
void FinalPlayScene::EarnMoney(int money) {
    this->money += money;
    UIMoney->Text = std::string("$") + std::to_string(this->money);
}
void FinalPlayScene::ReadMap() {
...
void FinalPlayScene::ReadEnemyWave() {
...
void FinalPlayScene::ConstructUI() {
    // Background
...
void FinalPlayScene::UIBtnClicked(int id) {
    if (preview)
        UIGroup->RemoveObject(preview->GetObjectIterator());
...
bool FinalPlayScene::CheckSpaceValid(int x, int y) {
...
std::vector<std::vector<int>> FinalPlayScene::CalculateBFSDistance() {
...
void FinalPlayScene::ReadNotes(int songID){
...
void FinalPlayScene::OnKeyUp(int keyCode){
    int lane = -1;
    for (int i = 0; i < lanes; ++i) {
        if (lane_key[i] == keyCode) {
            lane = i;
